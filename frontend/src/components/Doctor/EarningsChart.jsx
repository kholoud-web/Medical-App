...
function EarningsChart({type }) {
  return (
    
<>
<ChartContainer config={chartConfig}>
        <AreaChart width={285} height={170} data={chartData} margin={{ left: 0, right: 0 }}>
          <defs>
          ...
          <XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={3} tickFormatter={(v) => v.slice(0, 3)} />

          <YAxis tickLine={false} axisLine={false} tickMargin={3} />

          {/* <Tooltip /> */}
          <ChartTooltip content={<ChartTooltipContent indicator="dashed" />} cursor={false} />
          <Area dataKey="salary" type={type} fill="url(#salaryGradient)" stroke="#207EFF" />
        </AreaChart>
</ChartContainer>
   </>
  );
